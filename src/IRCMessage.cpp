...
/*   Updated: 2025/12/08 11:53:37 by elagouch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static std::string extractTrailing(std::string &line, size_t &pos) {
  if (!line.empty() && line[0] == ':') {
    std::string trailing = line.substr(1);
    line.clear();
    return trailing;
  }
...
const std::string &IRCMessage::getPrefix() const { return (this->_prefix); }

const std::string &IRCMessage::getCommand() const { return (this->_command); }

const std::vector<std::string> &IRCMessage::getParams() const {
  return (this->_params);
}

const std::string &IRCMessage::getTrailing() const { return (this->_trailing); }
...
size_t IRCMessage::getCountParams() const { return (this->_params.size()); }
