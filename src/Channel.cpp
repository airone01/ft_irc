...
/*   Updated: 2025/11/19 11:27:58 by elagouch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Channel::Channel(Client &tmp, int capacity) : _maxCapacity(capacity) {
  this->_users.insert(std::pair<int, Client *>(tmp.getSocket(), &tmp));
...
void Channel::tryJoin(const Client &tmp, std::string pswrd) {
  if (_modeSet) {
    ...
    if (_mode.find('l') != _mode.end())
      if ((static_cast<long>(_users.size()) == static_cast<long>(_maxCapacity)))
